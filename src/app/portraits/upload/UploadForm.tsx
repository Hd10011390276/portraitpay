    ...
    try {
      ...
      if (idCardFront) {
        ...
        if (!idCardRes.ok || !idCardJson.success) {
          ...
          const idCardFrontUrl = idCardJson.data.idCardFrontUrl ?? idCardJson.data.originalImageUrl ?? "";
      ...
      if (!s3Res.ok || !s3Json.success) {
        throw new Error(s3Json.error || t.upload?.uploadFailed);
      }
        ...
        <div className="max-w-3xl">
          <div className="text-center py-16">
            ...
            <h2 className="text-xl font-semibold mb-2">
              {stage === "uploading" ? (t.upload?.uploading) : (t.upload?.uploadSuccess)}
            </h2>
            <p className="text-gray-500">
              {stage === "uploading" ? (t.upload?.savingProgress) : (t.upload?.uploadRetryHint)}
            </p>
      ...
      <div className="max-w-3xl">
        <form onSubmit={handleSubmit} className="flex flex-col gap-8">
          ...
          <section className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{t.upload?.idCardSectionDesc}</p>

            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4 mb-4">
              <p className="text-sm text-amber-700 dark:text-amber-400">
                &#x26A0; <strong>{t.upload?.legalWarningTitle || "Important"}:</strong> {t.upload?.legalWarningDesc || "If the portrait does not match the ID card information, the certificate will not have legal effect."}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <select value={form.idCardType} onChange={e => { setForm(f => ({ ...f, idCardType: e.target.value })); setErrors(prev => ({ ...prev, idCardType: "" })); }}
              ...
              <div>
                ...
                <input type="text" value={form.idCardName} onChange={e => { setForm(f => ({ ...f, idCardName: e.target.value })); setErrors(prev => ({ ...prev, idCardName: "" })); }}
                  placeholder={t.upload?.idCardNamePlaceholder || "Enter name as on ID card"} maxLength={100}
                  className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-white" />
              ...
              <div className="sm:col-span-2">
                ...
                <input type="text" value={form.idCardNumber} onChange={e => { setForm(f => ({ ...f, idCardNumber: e.target.value })); setErrors(prev => ({ ...prev, idCardNumber: "" })); }}
                  placeholder={t.upload?.idCardNumberPlaceholder || "Enter ID number"} maxLength={50}
                  className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-white" />
            ...
            {!idCardFrontPreview ? (
              <label className="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl cursor-pointer hover:border-blue-400 dark:hover:border-blue-500 transition-colors"
                style={{ height: "200px" }}>
                <div className="text-4xl mb-3">&#x1F9AA;</div>
                <p className="text-sm font-medium text-gray-600 dark:text-gray-300">{t.upload?.clickToUploadIdFront}</p>
                <p className="text-xs text-gray-400 mt-1">{t.upload?.idDocSupported}</p>
                <input type="file" accept="image/jpeg,image/png,image/webp" className="hidden"
                  onChange={(e) => { const file = e.target.files?.[0]; if (file) handleIdCardFrontChange(file); setErrors(prev => ({ ...prev, idCardFront: "" })); }} />
              </label>
            ) : (
              <div className="flex flex-col gap-3">
                <div className="relative inline-block">
                  <img src={idCardFrontPreview} alt={t.upload?.idCardFront}
                    className="w-full max-w-sm rounded-xl border border-gray-200 dark:border-gray-700" style={{ maxHeight: "200px", objectFit: "cover" }} />
                  <button type="button" onClick={handleIdCardFrontRemove}
                    ...
                    title={t.upload?.remove}>&#x2715;</button>
                </div>
                <p className="text-xs text-green-600 dark:text-green-400">&#x2705; {t.upload?.idFrontUploaded}</p>
              </div>
            )}
          ...
          <section className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
            ...
            <div>
              ...
              <input type="text" value={form.title} onChange={e => { setForm(f => ({ ...f, title: e.target.value })); setErrors(prev => ({ ...prev, title: "" })); }}
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <button type="submit" disabled={stage === "uploading"}
              ...
              {stage === "uploading" ? (t.upload?.submitting) : (t.upload?.createPortrait)}
            </button>
            {stage === "uploading" && (
